...
def train_one_epoch(model: torch.nn.Module, criterion: DistillationLoss,
    ...
    print_freq = 10
    sab_stat = {"features": None, "cnt": None}
    ...
    for batch_index, (samples, targets, _) in enumerate(metric_logger.log_every(data_loader, print_freq, header)):
        ...
        if sam is not None and (args.sam_first == 'memory' and task_id > 0):
            ...
            with torch.cuda.amp.autocast(enabled=not args.no_amp):
                loss_tuple = forward(x, y, model, teacher_model, criterion, lam, args, sab_stat=sab_stat)
        else:
            with torch.cuda.amp.autocast(enabled=not args.no_amp):
                loss_tuple = forward(samples, targets, model, teacher_model, criterion, lam, args, sab_stat=sab_stat)
    ...
    return {k: meter.global_avg for k, meter in metric_logger.meters.items()}, sab_stat
...
def forward(samples, targets, model, teacher_model, criterion, lam, args, sab_stat=None):
    ...
    if isinstance(outputs, dict):
        ...
        if args.dynamic_tokens:
            ...
            sab_feature = outputs['sab_feature']
            if sab_stat["features"] is None:
                sab_stat["features"] = torch.zeros(nb_classes, sab_feature.shape[1], sab_feature.shape[2]).cuda()
                sab_stat["cnt"] = [0 for _ in range(nb_classes)]
            for c in range(nb_classes):
                pos = targets == c
                if pos.sum() > 0:
                    sab_stat["features"][c] += sab_feature[pos].sum(dim=0).detach()
                    sab_stat["cnt"][c] += pos.sum()
    else:
        main_output = outputs
...
@torch.no_grad()
def evaluate(data_loader, model, device, logger, sigmoid_logits=False):
...
def eval_and_log(args, output_dir, model, model_without_ddp, optimizer, lr_scheduler,
    ...
    test_stats = evaluate(data_loader_val, model, device, logger, sigmoid_logits=args.sigmoid_logits)
