...
def train_one_epoch(model: torch.nn.Module, criterion: DistillationLoss,
    ...
    for batch_index, (samples, targets, _) in enumerate(metric_logger.log_every(data_loader, print_freq, header)):
        ...
        #    print("Disp current model!")
        #    model_without_ddp.check()
        #    print("Disp teacher model!")
        #    teacher_model.check()
        if batch_index == 0:
...
def evaluate(data_loader, model, device, logger):
    ...
    for images, target, task_ids in metric_logger.log_every(data_loader, 10, header):
        ...
        with torch.cuda.amp.autocast():
            ...
            #    if tokens is None:
            #        tokens = [[] for _ in range(len(output['tokens']))]
            #    for i in range(len(output['tokens'])):
            #        tokens[i].append(output['tokens'][i].detach().cpu())
    ...
    #    tokens[i] = torch.cat(tokens[i], dim=0)
    #logits = torch.cat(logits, dim=0)
    #task_id = len(tokens)
    #import numpy as np
    #np.save("np_save/targets_task_{}".format(task_id), targets.numpy())
    #np.save("np_save/logits_task_{}".format(task_id), logits.numpy())
    #for i in range(task_id):
    #    np.save("np_save/tokens_{}_task_{}".format(i, task_id), tokens[i].numpy())
