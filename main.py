...
def get_args_parser():
    ...
    parser.add_argument('--dynamic-tokens', default=False, action='store_true',
                        help='Dynamically update token using the memory')
    parser.add_argument('--sigmoid-logits', default=False, action='store_true',
                        help='Add sigmoid to logits in evaluation')

    # Dynamic expanding related
    parser.add_argument('--split', action='store_true', default=False,
                        help='Dynamically expand the feature')
    parser.add_argument('--split-tab', action='store_true', default=False,
                        help='Set split in TAB')
    parser.add_argument('--extra-dim', default=32, type=int,
                        help='Extra embedding dim addded for each task')
    parser.add_argument('--extra-heads', default=1, type=int,
                        help='Extra attention heads addded for each task')
    parser.add_argument('--single-token', action='store_true', default=False,
                        help='Use only one token (but expandable)')
    parser.add_argument('--default-split', type=bool, default=True,
                        help='default_split in split block')
    parser.add_argument('--default-stack', type=bool, default=True,
                        help='default_stack in split block')
    parser.add_argument('--dense-mode', default=[], type=list,
                        help='Use dense mode in which blocks')
    parser.add_argument('--split_block-config', type=dict, default={},
                        help='Split block config')
...
def main(args):
    ...
    for task_id, dataset_train in enumerate(scenario_train):
        ...
        # If skipped task, model needs to update attention moduel before loading the checkpoint
        if args.model == "convit_split_v2" and args.resume and args.start_task > task_id:
            model_without_ddp.fix_and_update_attn()
        ...
        for epoch in range(initial_epoch, epochs):
            ...
            #train_stats, sab_stat = train_one_epoch(
            ...
            # Update class mean if use dynamic token
            #if args.dynamic_tokens:
            #    sab_nb_classes = len(sab_stat["cnt"])
            #    sab_features = sab_stat["features"]
            #    for c in range(sab_nb_classes):
            #        sab_features[c] /= sab_stat["cnt"][c]
            #    model_without_ddp.update_class_means(sab_features)
        ...
        if args.model == "convit_split_v2" and not skipped_task:
            model_without_ddp.fix_and_update_attn()
        ...
        #for my_i in range(task_id + 1):
        #    np.save("np_save/w_{}_{}".format(my_i, task_id), model_without_ddp.head[my_i].head.weight.detach().cpu().numpy())
        #    np.save("np_save/b_{}_{}".format(my_i, task_id), model_without_ddp.head[my_i].head.bias.detach().cpu().numpy())
        logger.end_task()
